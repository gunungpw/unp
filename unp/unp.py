...
class UnpackerBase(object):
    ...
    def assert_available(self):
        ...
        msgs = [
            'Cannot unpack "{}" because {} is not available.'.format(
    ...
    def unpack(self, dst):
        ...
        try:
            if self.real_unpack(tmp_dir) != 0:
                click.secho(
                    "Error: unpacking through %s failed." % self.executable, fg="red"
                )
                sys.exit(2)
    ...
    def dump_command(self, dst):
        ...
        for idx, arg in enumerate(args):
            if arg.split() != [arg]:
                args[idx] = '"%s"' % arg.replace("\\", "\\\\").replace('"', '\\"')
        click.echo(" ".join(args))

    def __repr__(self):
        return "<Unpacker {!r}>".format(
            self.name,
        )
...
class TarXZUnpacker(UnpackerBase):
    ...
    def real_unpack(self, dst):
        ...
        tar = subprocess.Popen(
            ["tar", "x"], cwd=cwd, stderr=subprocess.PIPE, stdin=subprocess.PIPE
        )
...
class DMGUnpacker(UnpackerBase):
    ...
    def real_unpack(self, dst):
        ...
        with open("/dev/null", "wb") as devnull:
            rv = subprocess.Popen(args, cwd=cwd, stdout=devnull, stderr=devnull).wait()
            if rv != 0:
                return rv

        p = subprocess.Popen(["cp", "-vpR", mp + "/", dst], stdout=subprocess.PIPE)
        while 1:
    ...
    def cleanup(self, dst):
        with open("/dev/null", "wb") as devnull:
            subprocess.Popen(
                ["umount", dst + "---mp"], stderr=devnull, stdout=devnull
            ).wait()
